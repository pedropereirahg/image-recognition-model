...
def variable_summaries(var):
  ...
  with tf.name_scope('summaries'):
    ...
    tf1.summary.scalar('mean', mean)
    with tf.name_scope('stddev'):
      stddev = tf.sqrt(tf.reduce_mean(tf.square(var - mean)))
    tf1.summary.scalar('stddev', stddev)
    tf1.summary.scalar('max', tf.reduce_max(var))
    tf1.summary.scalar('min', tf.reduce_min(var))
    tf1.summary.histogram('histogram', var)
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  with tf.name_scope('input'):
    bottleneck_input = tf1.placeholder_with_default(
        bottleneck_tensor, shape=[None, BOTTLENECK_TENSOR_SIZE],
        name='BottleneckInputPlaceholder')

    ground_truth_input = tf1.placeholder(tf.float32,
                                        [None, class_count],
  ...
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('Wx_plus_b'):
      ...
      tf1.summary.histogram('pre_activations', logits)

  final_tensor = tf.nn.softmax(logits, name=final_tensor_name)
  tf1.summary.histogram('activations', final_tensor)
  ...
  tf1.summary.scalar('cross_entropy', cross_entropy_mean)

  with tf.name_scope('train'):
    optimizer = tf1.train.GradientDescentOptimizer(FLAGS.learning_rate)
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
  ...
  tf1.summary.scalar('accuracy', evaluation_step)
...
def main(_):
  ...
  with tf1.Session(graph=graph) as sess:
    ...
    train_writer = tf1.summary.FileWriter(FLAGS.summaries_dir + '/train',
                                         sess.graph)

    validation_writer = tf1.summary.FileWriter(
